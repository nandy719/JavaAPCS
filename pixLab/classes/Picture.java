...
public void myCollage() {
	Picture geta1 = new Picture("gogeta.jpg");
	Picture geta2 = new Picture("gogeta.jpg");
	geta2.negate();
	geta1.negate();
	this.copy(geta1, 100, 0);
	this.copy(geta2, 69, 69);
	this.copy(geta2, 0, 0);
	this.zeroBlue();
	Picture getaMirror = new Picture("gogeta.jpg");
	getaMirror.mirrorDiagonal();
	this.copy(getaMirror, 0, 69);
	this.mirrorDiagonal();
  ...
  public void grayscale() {
	  ...
	  for(Pixel[] row : pixels) {
		  for(Pixel pix : row) {
			  double gray = pix.getAverage();
			  pix.setRed((int)gray);
			  pix.setBlue((int) gray);
			  pix.setGreen((int) gray);
		  }
	  }
  }
  public void fixUnderwater()
  {
    ...
    for (Pixel[] row : pixels)
    {
      for (Pixel pixel : row)
      {
        int red = Math.abs(pixel.getRed() - 21);
        int green = Math.abs(pixel.getGreen() - 163);
        int blue = Math.abs(pixel.getBlue() - 174);
